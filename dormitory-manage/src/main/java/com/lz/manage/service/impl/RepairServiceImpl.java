...
public class RepairServiceImpl extends ServiceImpl<RepairMapper, Repair> implements IRepairService {
    ...
    public List<Repair> selectRepairList(Repair repair) {
        List<Repair> repairs = repairMapper.selectRepairList(repair);
        for (Repair info : repairs) {
            ...
            SysUser withUser = sysUserService.selectUserById(info.getDealWithId());
            if (StringUtils.isNotNull(withUser)) {
                info.setDealWithName(withUser.getUserName());
            }
        }
        return repairs;
    }
    ...
    public int insertRepair(Repair repair) {
        checkRepair(repair);
        repair.setStatus(RepairStatusEnum.REPAIR_STATUS_0.getValue());
        repair.setUserId(SecurityUtils.getUserId());
        repair.setCreateTime(DateUtils.getNowDate());
        return repairMapper.insertRepair(repair);
    }

    private void checkRepair(Repair repair) {
        //校验宿舍是否是此栋楼
        if (StringUtils.isNotNull(repair.getDormitoryId())) {
            Dormitory dormitory = dormitoryService.selectDormitoryById(repair.getDormitoryId());
            if (StringUtils.isNull(dormitory)) {
                throw new ServiceException("宿舍不存在");
            }
            if (!dormitory.getBuildingId().equals(repair.getBuildingId())) {
                throw new ServiceException("宿舍不属于此栋楼");
            }
        }
        //判断床位是否是这栋楼，这个宿舍
        if (StringUtils.isNotNull(repair.getBedId())) {
            DormitoryBed dormitoryBed = dormitoryBedService.selectDormitoryBedById(repair.getBedId());
            if (StringUtils.isNull(dormitoryBed)) {
                throw new ServiceException("床位不存在");
            }
            if (!dormitoryBed.getDormitoryId().equals(repair.getDormitoryId())) {
                throw new ServiceException("床位不属于此宿舍");
            }
            if (!dormitoryBed.getBuildingId().equals(repair.getBuildingId())) {
                throw new ServiceException("床位不属于此栋楼");
            }
        }
    }
